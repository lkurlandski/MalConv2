"""
Old assets that may be useful to look at for building new assets.
"""
...
from executable_helper import get_bounds
from typing_ import Pathlike

####################################################################################################
# Modify malware by altering its entire .text section in one swoop.
####################################################################################################
...
####################################################################################################
# Determine which architectures a particular malware executable can be disassembled with.
####################################################################################################
...
class Disassembler:
    ...
    arch_modes: tp.ClassVar[tp.Dict[int, tp.List[int]]] = {
        cs.CS_ARCH_X86: (cs.CS_MODE_16, cs.CS_MODE_32, cs.CS_MODE_64),
    ...
    def __init__(
        self,
        bounds: tp.Dict[str, tp.Tuple[int, int]],
    ...
    def from_file(
        cls,
        text_section_bounds_file: Pathlike = None,
        arch: int = cs.CS_ARCH_X86,
        mode: int = cs.CS_MODE_32,
        *,
        verbose: bool = False,
    ) -> Disassembler:
        return cls.__init__(get_bounds(text_section_bounds_file))

    def write(self, outfile: Pathlike) -> Disassembler:
        with open(outfile, "w") as handle:
            json.dump(self.valid_arch_modes, handle, indent=4)

    @classmethod
    def get_arch_modes(cls, bytes_: str) -> tp.Dict[int, tp.List[int]]:
        successful = defaultdict(list)
        for arch, modes in cls.arch_modes.items():
            for m in modes:
                md = cs.Cs(arch, m)
    ...
    @staticmethod
    def get_bytes(file: Pathlike, l: int, u: int) -> str:
